...
import com.netdata.ArchiveFriendsRequest;
...
public class ArchiveWindow extends JFrame implements ActionListener,
	...
	private ArchiveFriendsResponse archiveFriendsResponse;

	public ArchiveWindow(GUIThread guiThread) {
		this.guiThread = guiThread;
		this.archiveFriendsResponse = archiveFriendsResponse;
		initComponents();

		guiThread.getwThread().unblock(new ArchiveFriendsRequest());
	...
	private void initComponents() {
		...
		{
			/*
			 * progressBarPanel.setLayout(new GridBagLayout()); ((GridBagLayout)
			 * progressBarPanel.getLayout()).columnWidths = new int[] { 0, 206,
			 * 0, 0 }; ((GridBagLayout) progressBarPanel.getLayout()).rowHeights
			 * = new int[] { 0, 0, 0, 0 }; ((GridBagLayout)
			 * progressBarPanel.getLayout()).columnWeights = new double[] { 1.0,
			 * 0.0, 1.0, 1.0E-4 }; ((GridBagLayout)
			 * progressBarPanel.getLayout()).rowWeights = new double[] { 1.0,
			 * 0.0, 1.0, 1.0E-4 };
			 */
			// ---- progressBar ----
	...
	private Vector<FriendPanelEntry> getFriendsPanels() {
		...
		for (int i = 0; i < archiveFriendsResponse.getFriendsFullName().size(); i++) {
			String friendFullName = archiveFriendsResponse.getFriendsFullName()
					.get(i);

			friends.add(new FriendPanelEntry(friendFullName, new ImageIcon()));
			System.out.print(friendFullName + ":");
		}
		System.out.println();
	...
	public void archiveFriends(ArchiveFriendsResponse resp) {
		this.archiveFriendsResponse = resp;
		...
		System.out
				.println("[ArchiveWindow] archiveFriends from ArchiveWindow.");
	}
	...
	public static void main(String[] args) {
		...
		ArchiveWindow a = new ArchiveWindow(null);
		a.setVisible(true);
